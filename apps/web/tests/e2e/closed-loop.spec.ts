...
test("从创建、进度、报告和三格式导出到历史重开形成 Mock 闭环", async ({ page }) => {
  ...
  const researchResponses: string[] = [];
  ...
  page.on("response", (response) => {
    if (response.url().includes("/api/research")) {
      researchResponses.push(`${response.status()} ${response.request().method()} ${response.url()}`);
    }
  });
  ...
  const reportPath = `/research/${phase3ResearchId}/reports/1`;
  const reportLink = page.getByRole("link", { name: "打开研究报告" });
  await expect(reportLink).toHaveAttribute("href", reportPath);
  ...
  try {
    ...
    throw new Error(
      `Report view did not render. URL=${page.url()}\n` +
        `Responses=${JSON.stringify(researchResponses)}\n` +
  ...
  await expect(page.getByRole("heading", { name: "AI 分析总结" })).toBeVisible();
  await expect(page.getByText("前景：中性观察")).toBeVisible();
  await expect(page.getByText("走势：更可能震荡")).toBeVisible();
  await expect(page.getByRole("heading", { name: "未来三种可能情景" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "乐观情景", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "基准情景", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "谨慎情景", exact: true })).toBeVisible();

  await page.getByRole("button", { name: /查看依据 1：ev_MU_RETURN_01/ }).first().click();
  ...
  await page.getByLabel("搜索公司公告内容").fill("supply risk");
  ...
  await page.getByText("专业审计信息", { exact: true }).click();
  ...
  const reopenLink = page.getByRole("link", { name: "打开报告" });
  await expect(reopenLink).toHaveAttribute("href", reportPath);
