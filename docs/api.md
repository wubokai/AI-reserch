...
- 认证：生产环境使用 Bearer Token。开发环境可启用演示用户，但不得因此改变响应结构。
...
`query` 必填；`symbol` 与 `companyName` 至少提供一个。`locale` 可选且只接受 `zh-CN | en-US`；未提供时服务端根据 `query` 语言确定并把最终值持久化，不能使用用户时区推断语言。若证券两项均提供但无法解析为同一证券，返回 `422 SECURITY_MISMATCH`。省略时 `period=5y`、`reportDepth=STANDARD`，三个分析开关均为 `true`。MVP 只接受美国上市普通股和 ETF。
...
- `POST /research/{id}/cancel` 是协作式取消：成功接收返回 `202`，任务可能短暂处于原执行状态并带 `cancellationRequested=true`，最终进入 `CANCELLED`。
- 已处于目标终态的 retry/cancel 返回 `409 INVALID_STATE_TRANSITION`；相同幂等请求重放除外。
...
| `404` | `RESEARCH_NOT_FOUND`, `REPORT_VERSION_NOT_FOUND` | 资源不存在或已软删除 |
