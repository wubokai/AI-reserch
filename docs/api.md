# Java REST API 契约（Phase 3 Mock 闭环）
...
- 认证：Phase 3 继续只在 development/test profile 下支持显式启用的 Basic 演示用户。Bearer Token 是生产契约，但正式接入延后；在此之前 production profile 启动即失败关闭，不能退化成 Basic 或匿名访问。
...
`progress` 范围为 0–100，且在一次执行中不得倒退。`PARTIALLY_COMPLETED` 只表示“已原子发布通过安全验证、但缺少可降级模块的报告”；不允许以已知错误或 unsupported material Claim 冒充部分成功。重试从首个失败或未完成步骤继续，已经成功且输入/实现哈希未变化的步骤不得重新执行。
...
Phase 3 普通研究流程固定为 `MOCK DATA ONLY`：Provider fixture、Python 计算和 Mock 报告生成都是确定性的，不需要真实数据 Key 或 `OPENAI_API_KEY`。缺少真实 Provider 凭据不得触发隐式模式切换，演示数据也不得伪装成真实或当前市场数据。
...
- `locale` 只接受 `zh-CN | en-US`；省略时根据 `query` 语言确定并持久化。

超出上述 Phase 3 边界的请求在创建任务前返回 `400 INVALID_REQUEST`，不会产生 Research。已知 symbol/companyName 矛盾仍返回 `422 SECURITY_MISMATCH`。创建请求线程不调用外网。
...
- `POST /research/{id}/retry` 仅允许 `FAILED` 或 `PARTIALLY_COMPLETED`。请求体可指定 `fromStep`；服务端会拒绝跳过必需依赖的请求。已成功且 input/implementation hash 未改变的前置步骤保持不变，从首个可运行失败步骤续跑；历史 attempt 和已发布报告版本不覆盖。
...
- 只导出当前用户可见的已发布不可变版本；`MIXED_TEST` 报告由数据库发布/导出守卫阻断。
- 响应设置安全的 `Content-Disposition`、与格式匹配的 `Content-Type`、`ETag`、`X-Content-SHA256`、`X-Report-Version` 和 `X-Data-Mode`。
- 相同报告版本、格式与渲染器版本产生确定性字节；所有 Mock 导出持续显示 `DEMO DATA - NOT REAL MARKET DATA`。
- HTML 对报告内容严格转义且不包含脚本或外部资源；PDF 只使用本地字体/资源并支持中文。
- Research 不存在、已软删除、不属于当前用户、报告尚未发布，或指定版本不存在时，统一返回 owner-scoped `404 RESEARCH_NOT_FOUND`，不泄露资源状态。任一格式的渲染/缓存校验失败返回可重试的 `503 EXPORT_RENDER_FAILED`。
...
| `404` | `ROUTE_NOT_FOUND`, `RESEARCH_NOT_FOUND` | 路由不存在，或 owner-scoped Research/已发布报告版本不可见 |
| `409` | `INVALID_STATE_TRANSITION`, `IDEMPOTENCY_KEY_REUSED` | 当前资源状态冲突 |
...
| `503` | `PROVIDER_UNAVAILABLE`, `EXPORT_RENDER_FAILED`, `SERVICE_UNAVAILABLE` | 临时不可用，可按 `retryable` 重试 |
