...
实现说明：`1y/3y/5y`、最长五年的显式日期范围以及 `QUICK/STANDARD/DEEP` 已实现。深度策略按层级限制 Filing、Evidence、确定性计算输入和 LLM 工具轮次；全局环境上限仍是不可突破的硬上限。
...
- 单次执行默认最多 15 分钟；到达执行截止时间后停止后续步骤且不得发布刚完成的过期结果。
