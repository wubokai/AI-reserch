...
## 11. Provider-neutral REAL 编排本地检查点

| 要求 | 自动化证据 | 结果 |
| --- | --- | --- |
| 创建边界 | REAL 接受格式合法的非 Mock ticker；MOCK 仍限制固定 fixture | 通过 |
| Security 隔离 | REAL 只查询 active/non-demo security master，缺失或歧义明确失败 | 通过 |
| 可选基本面 | REAL 缺失基本面时使用无数值、无 watermark 的显式空输入 | 通过 |
| 验证输出 | REAL 数据验证不写入 Mock watermark，warning 文案模式中立 | 通过 |
| 发布隔离 | 任务、报告、Source、Evidence 的 REAL/MOCK 模式必须一致 | 通过 |
| 测试模式 | `MIXED_TEST` 不能发布普通用户报告 | 通过 |
| 不可变记录 | report version 与 run manifest 使用任务 dataMode 和 phase7 policy | 通过 |

完整 REAL 闭环仍需合规 Market Adapter 和非 Demo security master 注册来源。
