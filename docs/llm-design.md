...
真实实现使用 Responses wire contract；Mock 实现是默认开发路径。生产当前选择经明确风险接受的 LanYi
兼容 endpoint，精确 host allowlist 只允许 `https://lanyiapi.com[/v1]`，不会开放任意自定义 URL。模型名、
推理强度和输出预算全部由环境变量/版本化配置控制，业务代码不硬编码具体模型。
...
- 审计 provider 根据 endpoint 记录为 `OPENAI` 或 `LANYI`；LanYi 会处理发送的 Evidence Pack，不能被
  描述为 OpenAI 官方数据处理边界。
...
生产参考配置按项目负责人选择使用 `gpt-5.5/xhigh`，但 LanYi 的模型映射、可用性和计价必须以其账户
当时页面为准。缺少准确的 input/cached/output 价格时 `LLM_PRICING_UNKNOWN` 会阻止真实网络调用。
