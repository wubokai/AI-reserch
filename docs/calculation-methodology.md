...
情景引擎根据盈利状态使用显式 `valuationMethod`：盈利公司使用 `EV_EBITDA`，未盈利公司使用
`EV_REVENUE`，避免把负 EBITDA 强行截成零价格。两种方法都以当前企业价值对应的市场倍数为
基准，再施加版本化的 Bull/Base/Bear 倍数与收入增长假设：

```text
forecastRevenue = baseRevenue * (1 + revenueGrowth)
forecastEbitda = forecastRevenue * targetEbitdaMargin
impliedEnterpriseValue = valuationMethod == EV_REVENUE
  ? forecastRevenue * valuationMultiple
  : forecastEbitda * valuationMultiple
...
- `targetEbitdaMargin` 可为负；只有显式 `EV_EBITDA` 方法遇到非正 EBITDA 才输出限制 warning，
  `EV_REVENUE` 不把负利润率当作零价格；
