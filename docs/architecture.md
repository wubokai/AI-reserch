...
| 对象 | 责任 | 关键约束 |
| --- | --- | --- |
| `research_jobs` | 一次 Research 的请求快照、公共阶段、进度和取消信号 | 公共状态使用既定枚举；终态不可逆；`row_version` 防并发覆盖 |
...
它是面向用户的单调阶段投影；内部步骤可按依赖并行执行。`research_steps` 只使用 `PENDING | RUNNING | SUCCEEDED | FAILED | SKIPPED | CANCELLED`。等待重试不增加新持久状态：表现为 `PENDING` 且 `available_at > now()`；UI 可派生显示 `WAITING_FOR_RETRY`。
...
若成功终态事务先于取消提交，取消幂等返回既有终态；若取消 flag 先提交，晚到的成功提交必须因条件谓词失败。已完成的内部 artifact 可保留审计，但取消后的 Research 不改判为 `PARTIALLY_COMPLETED`。
