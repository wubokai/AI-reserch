...
flowchart TB
    ...
    subgraph JAVA["Java 模块化单体 / Orchestrator"]
        API["API、鉴权与幂等"]
        EXECUTOR["Durable Executor<br/>claim、heartbeat、checkpoint"]
    ...
    subgraph PYTHON["无状态 Python Analytics"]
        RUNTIME["版本化内部 HTTP API"]
    ...
    GATEWAY --> PROVIDERS
    EXECUTOR -->|"受限 queue_v1 数据库函数"| PG
    EXECUTOR -.->|"缓存与取消加速提示"| REDIS
    EXECUTOR --> PROVIDERS
    EXECUTOR -->|"版本化分析请求"| RUNTIME
...
3. Java Durable Executor 只通过版本化 `queue_v1` 数据库函数改变当前 lease 授权的 `research_steps`、`step_attempts`、checkpoint 与步骤结果；Python Analytics 为无状态内部计算服务，不访问 PostgreSQL、Redis、Provider 或 LLM。执行边界见 [ADR-0008](./adr/0008-java-durable-executor-stateless-analytics.md)。
...
apps/analytics/                   # 无状态 Python Analytics API 与分析包
...
Java Durable Executor 使用 `FOR UPDATE SKIP LOCKED` 领取到期步骤，并在同一短事务中把步骤置为 `RUNNING`、递增 attempt 计数、创建 `step_attempts`、签发随机 `lease_token`。计算期间不持有事务或锁；Python Analytics 只处理 Java 传入的版本化输入。
...
- Research 从首个活动阶段开始计算全局执行预算。Worker 在阶段投影、步骤调用和提交/发布前检查持久化的 `started_at`；到期是不可重试失败，因而不能把截止时间后返回的 Provider、Analytics 或 LLM 结果发布为成功。
...
Executor 先写 attempt 暂存区并计算输出哈希，再用当前 token 原子提交。`queue_v1.complete_step_and_advance` 同事务解锁下一个非 `SKIPPED` 步骤，并以实际输出哈希、后继 implementation version 与 payload version 确定性派生后继输入哈希。相同幂等键和相同哈希返回既有结果；相同键但哈希不同进入 `IDEMPOTENCY_CONFLICT`，不得静默覆盖。报告和 Claim 仍须由 Java 验证后生成不可变 `report_versions`。

Phase 2 历史基线不接受 policy boolean 作为成功发布证明，因此对 `COMPLETED/PARTIALLY_COMPLETED` 失败关闭。Phase 3 已落地 `report_versions` 与运行级 manifest 的同事务发布边界：只有已发布、通过安全验证且数据模式合法的不可变报告能驱动成功终态。`FAILED` 与取消收敛到 `CANCELLED` 仍不依赖报告发布。
...
- 内置 relay 使用 `FOR UPDATE SKIP LOCKED` 有界批量发布进程内领域事件，再标记 `published_at`；发送后标记前崩溃会重复投递，所以任何新增 listener 必须按 event ID 幂等。
...
- Java Executor 使用最小权限数据库角色，只能执行 `queue_v1` 函数；lease token 不进普通日志或前端，Python Analytics 不接收 lease token。
