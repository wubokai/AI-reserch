...
- API：Java 21 / Spring Boot 3.5，139 个 Surefire 与 38 个 Failsafe/Testcontainers 测试通过；Mock Provider、Analytics HTTP 错误分类、Worker heartbeat/自隔离、Evidence/Claim、报告验证、查询 API 和确定性导出均有自动化覆盖；
...
- Phase 3 详细的本地与远程证据见 [`phase3-test-matrix.md`](./phase3-test-matrix.md)。

## 当前限制

- 本机没有 Docker；Gate G3 的容器/Testcontainers/Compose 终验已由 GitHub Actions Linux runner 完成。
