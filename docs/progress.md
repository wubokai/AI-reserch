...
| Phase 5-9 | 未开始 | 按 `implementation-plan.md` 推进 |
...
- API：Java 21 / Spring Boot 3.5，144 个 Surefire 与 38 个 Failsafe/Testcontainers 测试通过；Mock Provider、Analytics HTTP 错误分类、Phase 4 契约/降级策略、Worker heartbeat/自隔离、Evidence/Claim、报告验证、查询 API 和确定性导出均有自动化覆盖；
...
- Phase 3 与 Phase 4 详细证据分别见 [`phase3-test-matrix.md`](./phase3-test-matrix.md) 和 [`phase4-test-matrix.md`](./phase4-test-matrix.md)。

## 当前限制

- 本机没有 Docker；Gate G3/G4 的容器、Testcontainers 和 Compose 终验已由 GitHub Actions Linux runner 完成。
