...
| Phase 7：真实数据源 | 进行中 | SEC Filing/FRED 已通过 CI；SEC Companyfacts/XBRL Fundamental Adapter、concept/accession lineage、修订/跨期黄金样例已完成本地检查点；Market 许可失败关闭；缓存/熔断/指标待完成 |
...
- API：Java 21 / Spring Boot 3.5，Phase 7 SEC XBRL 检查点 185 个 Surefire 测试通过；47 个 Failsafe/Testcontainers 已编译；黄金 fixture 覆盖修订、time travel、period/unit、跨期派生拒绝和 concept/accession 落库；
...
- 本机没有 Docker；Gate G3–G6 与 Phase 7 SEC 检查点的容器、Testcontainers 和 Compose 终验均由 GitHub Actions Linux runner 完成。
