...
Provider Runtime 检查点当前为 191 个 Surefire 测试通过、48 个 Failsafe/Testcontainers 已编译：SEC/FRED/XBRL 共用有界 Redis 缓存、只统计可重试故障的熔断器，以及 `provider.requests/cache/retries` Prometheus 指标。Redis 故障不会阻断真实取数，超大 Filing 快照会跳过缓存；TTL 被限制在大于零且不超过七天。
